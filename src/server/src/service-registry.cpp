...
DictationService *ServiceRegistry::dictation() const { return m_dictation.get(); }

void ServiceRegistry::setDictation(std::unique_ptr<DictationService> service) {
  m_dictation = std::move(service);
}
