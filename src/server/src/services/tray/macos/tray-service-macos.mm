...
NSImage *renderTrayImage() {
    ...
    CGImageRef cgImage = CGImageCreate(
        image.width(), image.height(), 8, 32, image.bytesPerLine(), colorSpace,
...
TrayServiceMacOS::TrayServiceMacOS(QObject *parent)
    : TrayService(parent), m_impl(std::make_unique<Impl>()) {
    ...
    m_impl->versionItem = [[NSMenuItem alloc] initWithTitle:@"Vicinae"
                                                     action:nil
                                              keyEquivalent:@""];
