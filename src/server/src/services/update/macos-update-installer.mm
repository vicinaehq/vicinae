    ...
    for (const auto &entry : fs::directory_iterator(mountPoint, ec)) {
        ...
        if (!appPath.empty()) {
            return std::unexpected(QCoreApplication::translate(
                "macos-update-installer", "Update image contains more than one app"));
        }
        appPath = entry.path();
    }
...
std::optional<QString> verifySignature(const fs::path &appPath, const std::string &teamId) {
    @autoreleasepool {
        ...
        NSString *requirementString = [NSString
            stringWithFormat:@"anchor apple generic and certificate leaf[subject.OU] = \"%s\"",
                             teamId.c_str()];
...
std::optional<QString> verifyBundleVersion(const fs::path &appPath,
...
void MacosUpdateInstaller::install(const std::filesystem::path &archive,
                                   const QString &expectedVersion) {
    if (!m_self) {
...
void MacosUpdateInstaller::performInstall(const std::filesystem::path &archive,
    ...
    const fs::path staged =
        m_self->bundlePath.parent_path() / std::format("{}{}.app", STAGED_PREFIX, getpid());
    ...
    @autoreleasepool {
        ...
        if (![fm copyItemAtPath:@(appPath->c_str()) toPath:@(staged.c_str()) error:&copyError]) {
            return failWith(tr("Failed to stage update: %1")
