...
using ProviderFactory = std::function<std::unique_ptr<AbstractProvider>(const ConfigValue::ProviderConfig &)>;

class ProviderRegistry {
public:
  void add(std::string type, ProviderFactory factory) { m_factories[std::move(type)] = std::move(factory); }

  std::unique_ptr<AbstractProvider> create(const ConfigValue::ProviderConfig &config) const {
