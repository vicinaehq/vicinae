...
AbstractWindowManager::WindowPtr buildWindow(AXUIElementRef element, pid_t pid,
                                             const QString &bundleId, const QString &appName) {
    ...
    return std::make_shared<MacosWindow>(element, std::move(id), std::move(title), bundleId, pid,
                                         axCopyBounds(element), canClose, fullScreen,
                                         canFullScreen);
}
...
void appendWindowDeduped(AXUIElementRef element, pid_t pid, const QString &bundleId,
                         const QString &appName, AbstractWindowManager::WindowList &out,
                         std::unordered_set<CGWindowID> &seen) {
    CGWindowID windowId = 0;
    if (_AXUIElementGetWindow(element, &windowId) == kAXErrorSuccess && windowId != 0) {
        if (!seen.insert(windowId).second) return;
    }
    if (auto window = buildWindow(element, pid, bundleId, appName)) {
...
void collectCurrentSpaceWindows(pid_t pid, const QString &bundleId, const QString &appName,
    ...
    if (AXUIElementCopyAttributeValue(appElement, kAXWindowsAttribute,
                                      reinterpret_cast<CFTypeRef *>(&axWindows)) ==
            kAXErrorSuccess &&
        axWindows) {
...
std::vector<AppDescriptor> runningRegularApps() {
    ...
    @autoreleasepool {
        ...
        for (NSRunningApplication *app in apps) {
            ...
            QString appName =
                app.localizedName ? QString::fromNSString(app.localizedName) : bundleId;
...
AbstractWindowManager::WindowList scanWindows(const std::vector<AppDescriptor> &apps) {
    ...
    auto deadline =
        std::chrono::steady_clock::now() + std::chrono::milliseconds(BRUTE_FORCE_TOTAL_BUDGET_MS);
...
std::vector<AbstractWindowManager::Screen> MacosWindowManager::listScreensSync(
    ...
    @autoreleasepool {
        ...
        for (NSScreen *nsScreen in nsScreens) {
            ...
            Screen screen{.name = QString::fromNSString(nsScreen.localizedName),
                          .bounds = QRect(QPoint(qRound(bounds.origin.x), qRound(bounds.origin.y)),
                                          logicalSize),
                          .physicalResolution = displayScanoutSize(display).value_or(
                              logicalSize * nsScreen.backingScaleFactor)};
...
AbstractWindowManager::WindowPtr MacosWindowManager::getFocusedWindowSync() const {
    ...
    @autoreleasepool {
        ...
        QString appName =
            front.localizedName ? QString::fromNSString(front.localizedName) : bundleId;
...
bool MacosWindowManager::closeWindow(const AbstractWindow &window) const {
    ...
    if (AXUIElementCopyAttributeValue(macWindow->element(), kAXCloseButtonAttribute,
    ...
    AXError err =
        AXUIElementPerformAction(static_cast<AXUIElementRef>(closeButton), kAXPressAction);
...
bool MacosWindowManager::setWindowBounds(const AbstractWindow &window,
    ...
    CGSize size{.width = static_cast<CGFloat>(bounds.width),
                .height = static_cast<CGFloat>(bounds.height)};
    ...
    bool posOk = AXUIElementSetAttributeValue(element, kAXPositionAttribute, positionValue) ==
                 kAXErrorSuccess;
    bool sizeOk =
        AXUIElementSetAttributeValue(element, kAXSizeAttribute, sizeValue) == kAXErrorSuccess;
