...
CustomCommandWallpaperBackend::setWallpaper(const WallpaperRequest &request) {
  return QtConcurrent::run([request, command = m_command]() -> std::expected<void, std::string> {
    if (command.empty()) return std::unexpected("No custom wallpaper command configured");
    ...
    for (const auto &token : QProcess::splitCommand(QString::fromStdString(command))) {
    ...
    if (!command.contains("{path}")) { resolved << QString::fromStdString(request.path); }
