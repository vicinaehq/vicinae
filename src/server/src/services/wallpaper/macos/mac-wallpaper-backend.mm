    ...
    @autoreleasepool {
        ...
        if (request.screen) {
            ...
            for (NSScreen *screen in targets) {
                if (wanted && [screen.localizedName isEqualToString:wanted]) {
                    [matched addObject:screen];
                }
            }
...
QFuture<std::expected<void, std::string>> MacWallpaperBackend::setWallpaper(
    const WallpaperRequest &request) {
