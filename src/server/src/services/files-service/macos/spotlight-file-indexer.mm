...
std::string anyOf(std::initializer_list<std::string_view> predicates) {
    auto predicate = predicates | std::views::join_with(std::string_view{" || "}) |
...
std::optional<std::string> categoryPredicate(vicinae::FileCategory category) {
    switch (category) {
        ...
        case vicinae::FileCategory::Document:
            return anyOf(
                {R"(kMDItemContentTypeTree == "public.text")",
                 R"(kMDItemContentTypeTree == "public.rtf")",
...
std::string buildPredicate(std::string_view query, const IndexerQueryParams &params) {
    ...
    if (predicate.empty() || !params.category) {
        return predicate;
    }
...
std::vector<IndexerFileResult> runQuery(const std::string &query,
    ...
    for (CFIndex i = 0; i < count; ++i) {
        ...
        if (CFStringGetCString(pathRef, buf, sizeof(buf), kCFStringEncodingUTF8)) {
            ...
            int const fuzzyScore =
                matcher.fuzzy_match_v2_score_query(path.filename().string(), query);
...
QFuture<std::vector<IndexerFileResult>> SpotlightFileIndexer::queryAsync(
    std::string_view query, const IndexerQueryParams &params) {
