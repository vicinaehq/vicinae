...
void injectText(std::string_view text, int delayUs) {
    ...
    for (int off = 0; off < total;) {
        ...
        // in two
        if (off + count < total && QChar::isHighSurrogate(units[off + count - 1])) {
            --count;
        }
...
CGEventRef tapCallback(CGEventTapProxy, CGEventType type, CGEventRef event, void *refcon) {
    ...
    if (CGEventGetIntegerValueField(event, kCGEventSourceUserData) == VICINAE_EVENT_TAG) {
        return event;
    }
    ...
    const std::string utf8 =
        QString::fromUtf16(reinterpret_cast<const char16_t *>(chars), static_cast<int>(len))
            .toStdString();
...
void MacosSnippetServer::runTap() {
    ...
    if (!tap) {
        qWarning()
            << "MacosSnippetServer: failed to create event tap (accessibility permission missing?)";
        return;
    }
...
void MacosSnippetServer::registerSnippet(snippet_gen::CreateSnippetRequest payload) {
    ...
    std::ranges::sort(m_snippets,
                      [](auto &&a, auto &&b) { return a.trigger.size() > b.trigger.size(); });
}
...
void MacosSnippetServer::injectExpand(const std::string &text, unsigned charsToDelete, unsigned,
...
void MacosSnippetServer::reenableTap() {
    if (m_tap) {
        CGEventTapEnable(static_cast<CFMachPortRef>(m_tap), true);
    }
}
...
void MacosSnippetServer::onKey(int keycode, const std::string &utf8, bool blockingMods) {
    ...
    if (keycode == VK_DELETE) {
        if (!m_text.empty()) {
            m_text.pop_back();
               ...
               std::isprint(static_cast<unsigned char>(utf8.front()))) {
        m_text.append(utf8);
        if (m_text.size() > MAX_BUFFER_SIZE) {
            m_text.erase(0, m_text.size() - MAX_BUFFER_SIZE);
        }
    }
    ...
    for (const auto &snippet : m_snippets) {
        if (snippet.mode == snippet_gen::ExpansionMode::Keydown) {
            if (snippet.trigger.size() > m_text.size()) {
                continue;
            }
            ...
            if (snippet.trigger.size() + 1 > m_text.size()) {
                continue;
            }
