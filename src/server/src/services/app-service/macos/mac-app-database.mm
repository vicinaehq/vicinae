...
NSString *toNSString(const QString &s) {
    return [NSString stringWithCharacters:reinterpret_cast<const unichar *>(s.utf16())
                                   length:s.size()];
}
...
ClassifiedTarget classifyTarget(const QString &target) {
    ...
    if (looksLikePath) {
        @autoreleasepool {
            NSString *expanded =
                [[NSString stringWithCharacters:reinterpret_cast<const unichar *>(target.utf16())
...
void collectEmbeddedApps(const fs::path &bundle, std::vector<fs::path> &out) {
    ...
    for (const auto &entry :
        ...
        if (entry.path().filename().string().ends_with(".app")) {
...
bool MacAppDatabase::launch(const AbstractApplication &app,
                            const std::vector<QString> &args) const {
...
bool MacAppDatabase::launchTerminalCommand(const std::vector<QString> &cmdline,
    ...
    if (opts.hold) {
        script += "echo; read -n 1 -s -r -p 'Press any key to close...'\n";
    }

    fs::path const scriptPath =
        fs::path(QString::fromNSString(NSTemporaryDirectory()).toStdString()) /
        (QString("vicinae-%1.command")
             .arg(QString::fromNSString([[NSUUID UUID] UUIDString]))
             .toStdString());
...
std::vector<MacAppDatabase::AppPtr> MacAppDatabase::list() const {
...
MacAppDatabase::AppPtr MacAppDatabase::genericTextEditor() const {
    ...
    @autoreleasepool {
        NSURL *appURL =
            [[NSWorkspace sharedWorkspace] URLForApplicationToOpenContentType:UTTypePlainText];
        id = bundleIdentifierAt(appURL);
    }
    return id.isEmpty() ? nullptr : findById(id);
}
...
MacAppDatabase::AppPtr MacAppDatabase::webBrowser() const {
    ...
    @autoreleasepool {
        NSURL *appURL =
            [[NSWorkspace sharedWorkspace] URLForApplicationToOpenContentType:UTTypeHTML];
        id = bundleIdentifierAt(appURL);
    }
    return id.isEmpty() ? nullptr : findById(id);
}
