...
void installEffectView(NSWindow *nswin, bool enabled, bool wantLiquidGlass,
    ...
    if (wantLiquidGlass && !glassCls) {
        ...
        if (!warned) {
            warned = true;
            qWarning()
                << "macos-chrome: liquidGlass requested but NSGlassEffectView is unavailable "
...
void MacOSWindowAttached::revert() {
    ...
    installEffectView(nswin, /*enabled=*/false, /*wantLiquidGlass=*/false,
                      NSVisualEffectMaterialHUDWindow, QString(), 0, QColor(), 0);
