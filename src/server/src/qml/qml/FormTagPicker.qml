...
Item {
    ...
    property var suggestionsModel: null
            ...
            TextInput {
                ...
                Keys.onPressed: event => {
                    if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter) {
                        // handling via the usual `Qt.NoModifier` messes with keypad Enter
                        let anySignificantModifierPressed = Qt.ShiftModifier | Qt.ControlModifier | Qt.AltModifier | Qt.MetaModifier;
                        if (event.modifier & anySignificantModifierPressed) {
                            console.log("REFUSED: Ctrl, Alt, Super, or Shift are pressed");
                            event.accepted = false;
                            return;
                        }
                        if (textInput.text.length == 0 || !root.suggestionsModel) {
                            event.accepted = false;
                            return;
                        }
                        var copy = pickedItems.slice();
                        var selectedId = root.suggestionsModel.itemValueAt(0);
                        if (!selectedId) {
                            event.accepted = false;
                            return;
                        }
                        ...
                        root.textEdited();
                        event.accepted = true;
                    } else if (event.key === Qt.Key_Backspace && text === "" && root.pickedItems.length > 0) {
                        repeater.itemAt(root.pickedItems.length - 1).forceActiveFocus();
                        event.accepted = true;
                    } else if (event.key === Qt.Key_Left && cursorPosition === 0 && root.pickedItems.length > 0) {
                        repeater.itemAt(root.pickedItems.length - 1).forceActiveFocus();
                        event.accepted = true;
                    }
                }
    ...
    Popup {
        ...
        visible: textInput.text.length > 0 && fuzzyResultList.count > 0

        x: 0
        y: border.height + 4
        width: border.width
        height: Math.min(fuzzyResultList.count * 30, 180)
        ...
        contentItem: ListView {
            ...
            delegate: Item {
                ...
                RowLayout {
                    ...
                    Loader {
                        active: iconSource !== undefined && iconSource !== ""
