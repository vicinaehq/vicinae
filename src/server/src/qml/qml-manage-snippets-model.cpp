...
QString QmlManageSnippetsModel::displayIconSource(const snippet::SerializedSnippet &item) const {
  const auto visitor =
...
QString QmlManageSnippetsModel::displayAccessory(const snippet::SerializedSnippet &item) const {
  if (item.expansion) return QString::fromStdString(item.expansion->keyword);
  return {};
}

std::unique_ptr<ActionPanelState>
QmlManageSnippetsModel::buildActionPanel(const snippet::SerializedSnippet &item) const {
...
void QmlManageSnippetsModel::itemSelected(const snippet::SerializedSnippet &item) {
  emit snippetSelected(item);
}

QString QmlManageSnippetsModel::sectionLabel() const {
