...
QString QmlBrowseAppsModel::displayTitle(const AppPtr &app) const { return app->displayName(); }

QString QmlBrowseAppsModel::displaySubtitle(const AppPtr &app) const { return app->description(); }

QString QmlBrowseAppsModel::displayIconSource(const AppPtr &app) const {
  return imageSourceFor(app->iconUrl());
}

QString QmlBrowseAppsModel::displayAccessory(const AppPtr &app) const {
  if (!app->displayable()) return QStringLiteral("Hidden");
  return {};
}

std::unique_ptr<ActionPanelState> QmlBrowseAppsModel::buildActionPanel(const AppPtr &app) const {
...
QString QmlBrowseAppsModel::sectionLabel() const {
