...
QmlRaycastStoreModel::QmlRaycastStoreModel(QObject *parent) : QmlCommandListModel(parent) {}

void QmlRaycastStoreModel::setEntries(const std::vector<Raycast::Extension> &extensions,
...
QHash<int, QByteArray> QmlRaycastStoreModel::roleNames() const {
...
QVariant QmlRaycastStoreModel::data(const QModelIndex &index, int role) const {
  int s, i;
...
QString QmlRaycastStoreModel::itemTitle(int, int i) const { return m_entries[i].extension.title; }

QString QmlRaycastStoreModel::itemSubtitle(int, int i) const {
  return m_entries[i].extension.description;
}

QString QmlRaycastStoreModel::itemIconSource(int, int i) const {
  return imageSourceFor(m_entries[i].extension.themedIcon());
}

std::unique_ptr<ActionPanelState> QmlRaycastStoreModel::createActionPanel(int, int i) const {
  ...
  auto showDetails = new StaticAction(
      ...
      [ext = entry.extension, ctx = this->ctx()]() {
        ctx->navigation->pushView(new QmlRaycastStoreDetailHost(ext));
      });
