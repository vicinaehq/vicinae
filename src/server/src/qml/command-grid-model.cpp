...
QHash<int, QByteArray> CommandGridModel::roleNames() const {
  return {
      {IsSection, "isSection"},
...
std::vector<CommandGridModel::FlatRow>
...
void CommandGridModel::setColumns(int cols) {
  if (cols < 1) cols = 1;
...
void CommandGridModel::setAspectRatio(double ratio) {
...
int CommandGridModel::totalItemCount() const {
  int total = 0;
...
int CommandGridModel::toGlobal(int section, int item) const {
  int idx = 0;
