#include "qml-text-selection-controller.hpp"
#include <QClipboard>
...
QmlTextSelectionController::QmlTextSelectionController(QObject *parent)
    : QObject(parent) {
  m_autoScrollTimer.setInterval(AUTO_SCROLL_INTERVAL_MS);
  connect(&m_autoScrollTimer, &QTimer::timeout, this,
          &QmlTextSelectionController::autoScrollTick);
}

void QmlTextSelectionController::setFlickable(QQuickItem *item) {
  if (m_flickable == item)
    return;
  if (m_flickable)
...
bool QmlTextSelectionController::eventFilter(QObject *obj, QEvent *event) {
  if (obj != m_flickable)
    return false;
  switch (event->type()) {
...
void QmlTextSelectionController::registerSelectable(QQuickItem *item, int order,
                                                    bool isTextEdit) {
  if (!item)
    return;
  if (isTextEdit)
...
void QmlTextSelectionController::unregisterSelectable(QQuickItem *item) {
  std::erase_if(m_entries, [item](const auto &e) { return e.item == item; });
}

QPointF QmlTextSelectionController::toContainerCoords(qreal viewportX,
...
void QmlTextSelectionController::refreshGeometry() {
  if (!m_container)
    return;
...
int QmlTextSelectionController::entryAt(qreal containerX,
...
int QmlTextSelectionController::positionAt(QQuickItem *textEdit,
                                           qreal containerX,
                                           qreal containerY) const {
  int result = 0;
...
QString QmlTextSelectionController::linkAt(QQuickItem *textEdit,
                                           qreal containerX,
                                           qreal containerY) const {
  QString result;
  auto localPos = textEdit->mapFromItem(m_container, QPointF(containerX, containerY));
...
void QmlTextSelectionController::handlePress(qreal x, qreal y) {
...
void QmlTextSelectionController::handleMove(qreal x, qreal y) {
...
void QmlTextSelectionController::handleRelease(qreal x, qreal y) {
  stopAutoScroll();
...
void QmlTextSelectionController::handleDoubleClick(qreal x, qreal y) {
  refreshGeometry();
...
void QmlTextSelectionController::selectAll() {
  if (m_entries.empty())
    return;
...
void QmlTextSelectionController::clearSelection() {
  for (auto &entry : m_entries) {
    if (entry.isTextEdit)
...
void QmlTextSelectionController::copy() {
  if (!m_hasSelection)
    return;
...
void QmlTextSelectionController::applySelection(int fromEntry, int fromChar,
...
void QmlTextSelectionController::selectTextEdit(QQuickItem *item, int start,
                                                int end) {
  QMetaObject::invokeMethod(item, "select", Q_ARG(int, start), Q_ARG(int, end));
}

void QmlTextSelectionController::selectAllTextEdit(QQuickItem *item) {
  QMetaObject::invokeMethod(item, "selectAll");
}

void QmlTextSelectionController::deselectTextEdit(QQuickItem *item) {
  QMetaObject::invokeMethod(item, "deselect");
}

void QmlTextSelectionController::setNonTextSelected(QQuickItem *item,
                                                    bool selected) {
  item->setProperty("selected", selected);
}

void QmlTextSelectionController::setHasSelection(bool has) {
...
void QmlTextSelectionController::startAutoScroll() {
  if (!m_autoScrollTimer.isActive())
    m_autoScrollTimer.start();
}

void QmlTextSelectionController::stopAutoScroll() {
  m_autoScrollTimer.stop();
}

void QmlTextSelectionController::autoScrollTick() {
