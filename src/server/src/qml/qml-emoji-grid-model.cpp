...
void QmlEmojiGridModel::initialize(ApplicationContext *ctx) {
  QmlCommandGridModel::initialize(ctx);
...
void QmlEmojiGridModel::regenerateMetaSections() {
  m_pinned.clear();
  m_recent.clear();
...
void QmlEmojiGridModel::rebuildSections() {
...
const EmojiData *QmlEmojiGridModel::emojiAt(int section, int item) const {
...
QString QmlEmojiGridModel::emojiIcon(int section, int item) const {
...
QString QmlEmojiGridModel::emojiName(int section, int item) const {
  auto *data = emojiAt(section, item);
  if (!data) return {};
