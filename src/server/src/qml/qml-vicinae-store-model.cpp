...
QmlVicinaeStoreModel::QmlVicinaeStoreModel(QObject *parent) : QmlCommandListModel(parent) {}

void QmlVicinaeStoreModel::setEntries(const std::vector<VicinaeStore::Extension> &extensions,
...
QHash<int, QByteArray> QmlVicinaeStoreModel::roleNames() const {
...
QVariant QmlVicinaeStoreModel::data(const QModelIndex &index, int role) const {
  int s, i;
...
QString QmlVicinaeStoreModel::itemTitle(int, int i) const { return m_entries[i].extension.title; }

QString QmlVicinaeStoreModel::itemSubtitle(int, int i) const {
  return m_entries[i].extension.description;
}

QString QmlVicinaeStoreModel::itemIconSource(int, int i) const {
  return imageSourceFor(ImageURL::http(m_entries[i].extension.themedIcon()));
}

std::unique_ptr<ActionPanelState> QmlVicinaeStoreModel::createActionPanel(int, int i) const {
  ...
  auto showDetails = new StaticAction(
      ...
      [ext = entry.extension, ctx = this->ctx()]() {
        ctx->navigation->pushView(new QmlVicinaeStoreDetailHost(ext));
      });
