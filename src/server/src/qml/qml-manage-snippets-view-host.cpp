...
QUrl QmlManageSnippetsViewHost::qmlComponentUrl() const {
  return QUrl(QStringLiteral("qrc:/Vicinae/DetailListView.qml"));
}

QVariantMap QmlManageSnippetsViewHost::qmlProperties() const {
  return {{QStringLiteral("host"),
           QVariant::fromValue(const_cast<QmlManageSnippetsViewHost *>(this))}};
}

void QmlManageSnippetsViewHost::initialize() {
  BaseView::initialize();

  m_snippetService = context()->services->snippetService();
  m_model = new QmlManageSnippetsModel(this);
  ...
  connect(m_snippetService, &SnippetService::snippetsChanged, this,
          &QmlManageSnippetsViewHost::reload);

  connect(m_model, &QmlManageSnippetsModel::snippetSelected, this,
          &QmlManageSnippetsViewHost::loadDetail);
...
void QmlManageSnippetsViewHost::loadInitialData() { reload(); }

void QmlManageSnippetsViewHost::textChanged(const QString &text) {
  m_model->setFilter(text);
}

void QmlManageSnippetsViewHost::onReactivated() {
  m_model->refreshActionPanel();
}

void QmlManageSnippetsViewHost::beforePop() {
  clearDetail();
  m_model->beforePop();
}

QObject *QmlManageSnippetsViewHost::listModel() const { return m_model; }
...
void QmlManageSnippetsViewHost::updateExpandedText() {
...
void QmlManageSnippetsViewHost::clearDetail() {
...
void QmlManageSnippetsViewHost::reload() {
