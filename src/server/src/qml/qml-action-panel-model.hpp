...
class QmlActionPanelModel : public QAbstractListModel {
  Q_OBJECT
...
public:
  ...
  explicit QmlActionPanelModel(QObject *parent = nullptr);
  explicit QmlActionPanelModel(std::unique_ptr<ActionPanelState> state, QObject *parent = nullptr);
  explicit QmlActionPanelModel(const ActionPanelState *state, QObject *parent = nullptr);
...
signals:
  ...
  void submenuRequested(QmlActionPanelModel *subModel);
