...
void QmlManageFallbackModel::applyFilter() {
...
bool QmlManageFallbackModel::isFallbackEnabled(const RootItemPtr &item) const {
  return std::ranges::find(m_fallbacks, item) != m_fallbacks.end();
}

QString QmlManageFallbackModel::displayTitle(const RootItemPtr &item) const {
  return item->displayName();
}

QString QmlManageFallbackModel::displaySubtitle(const RootItemPtr &item) const {
  return item->subtitle();
}

QString QmlManageFallbackModel::displayIconSource(const RootItemPtr &item) const {
  return imageSourceFor(item->iconUrl());
}

std::unique_ptr<ActionPanelState> QmlManageFallbackModel::buildActionPanel(const RootItemPtr &item) const {
