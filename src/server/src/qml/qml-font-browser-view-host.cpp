...
std::unique_ptr<ActionPanelState> QmlFontBrowserModel::buildActionPanel(const QString &item) const {
...
QVariantMap QmlFontBrowserViewHost::qmlProperties() const {
  return {{QStringLiteral("host"),
           QVariant::fromValue(const_cast<QmlFontBrowserViewHost *>(this))}};
}

void QmlFontBrowserViewHost::initialize() {
  BaseView::initialize();

  m_model = new QmlFontBrowserModel(this);
  m_model->initialize(context());

  setSearchPlaceholderText("Browse fonts to preview...");

  connect(m_model, &QmlFontBrowserModel::fontSelected, this, [this](const QString &family) {
    m_selectedFont = family;
    emit selectedFontChanged();
  });
}

void QmlFontBrowserViewHost::loadInitialData() {
  auto families = QFontDatabase::families();
  std::vector<QString> items(families.begin(), families.end());
  m_model->setItems(std::move(items));
}

void QmlFontBrowserViewHost::textChanged(const QString &text) { m_model->setFilter(text); }

void QmlFontBrowserViewHost::onReactivated() { m_model->refreshActionPanel(); }

void QmlFontBrowserViewHost::beforePop() { m_model->beforePop(); }

QObject *QmlFontBrowserViewHost::listModel() const { return m_model; }

QString QmlFontBrowserViewHost::showcaseMarkdown() const { return showcaseMarkdownContent; }
