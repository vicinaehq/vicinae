...
QmlActionPanelModel::QmlActionPanelModel(std::unique_ptr<ActionPanelState> state, QObject *parent)
    : QmlActionPanelModel(parent) {
  setState(std::move(state));
}

QmlActionPanelModel::QmlActionPanelModel(const ActionPanelState *state, QObject *parent)
    : QmlActionPanelModel(parent) {
  setStateFrom(state);
}

void QmlActionPanelModel::setState(std::unique_ptr<ActionPanelState> state) {
  setStateFrom(state.get());
  // The unique_ptr is released here — actions are kept alive by shared_ptrs in m_allActions
}

void QmlActionPanelModel::setStateFrom(const ActionPanelState *state) {
  beginResetModel();
...
int QmlActionPanelModel::rowCount(const QModelIndex &parent) const {
  if (parent.isValid()) return 0;
  return static_cast<int>(m_flat.size());
}

QVariant QmlActionPanelModel::data(const QModelIndex &index, int role) const {
...
QHash<int, QByteArray> QmlActionPanelModel::roleNames() const {
  return {
      {ItemType, "itemType"},
      {Title, "title"},
...
void QmlActionPanelModel::activate(int index) {
  ...
  if (action->isSubmenu()) {
    ...
    auto *subModel = new QmlActionPanelModel(std::move(subState), this);
...
void QmlActionPanelModel::setFilter(const QString &text) {
...
int QmlActionPanelModel::nextSelectableIndex(int from, int direction) const {
...
void QmlActionPanelModel::rebuildFlatList() {
  m_flat.clear();
