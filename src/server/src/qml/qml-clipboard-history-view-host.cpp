...
QVariantMap QmlClipboardHistoryViewHost::qmlProperties() const {
  return {{QStringLiteral("host"),
           QVariant::fromValue(const_cast<QmlClipboardHistoryViewHost *>(this))}};
}

void QmlClipboardHistoryViewHost::initialize() {
  BaseView::initialize();
  ...
  m_model->setDefaultAction(defaultActionStr == "paste"
                                ? QmlClipboardHistoryModel::DefaultAction::Paste
                                : QmlClipboardHistoryModel::DefaultAction::Copy);
...
void QmlClipboardHistoryViewHost::loadInitialData() {
  m_controller->setFilter(searchText());
}

void QmlClipboardHistoryViewHost::textChanged(const QString &text) {
  m_controller->setFilter(text);
}

void QmlClipboardHistoryViewHost::onReactivated() {
  m_model->refreshActionPanel();
}

void QmlClipboardHistoryViewHost::beforePop() {
  m_model->beforePop();
}

QObject *QmlClipboardHistoryViewHost::listModel() const { return m_model; }

void QmlClipboardHistoryViewHost::toggleMonitoring() {
  QJsonObject patch;
...
void QmlClipboardHistoryViewHost::setKindFilter(int kind) {
...
void QmlClipboardHistoryViewHost::handleMonitoringChanged(bool monitoring) {
  if (monitoring) {
...
void QmlClipboardHistoryViewHost::handleDataRetrieved(int totalCount) {
...
void QmlClipboardHistoryViewHost::clearDetail() {
...
void QmlClipboardHistoryViewHost::saveDropdownFilter(const QString &value) {
  command()->storage().setItem("filter", value);
}

std::optional<QString> QmlClipboardHistoryViewHost::getSavedDropdownFilter() {
