...
QmlPreferenceFormModel::QmlPreferenceFormModel(QObject *parent) : QAbstractListModel(parent) {
  m_saveTimer.setInterval(1s);
  m_saveTimer.setSingleShot(true);
  connect(&m_saveTimer, &QTimer::timeout, this, &QmlPreferenceFormModel::save);
}

QmlPreferenceFormModel::~QmlPreferenceFormModel() {
  if (m_saveTimer.isActive()) save();
}

int QmlPreferenceFormModel::rowCount(const QModelIndex &) const { return static_cast<int>(m_fields.size()); }

QVariant QmlPreferenceFormModel::data(const QModelIndex &index, int role) const {
...
QHash<int, QByteArray> QmlPreferenceFormModel::roleNames() const {
...
void QmlPreferenceFormModel::loadProvider(const QString &providerId, const std::vector<Preference> &preferences) {
  beginResetModel();
...
void QmlPreferenceFormModel::setFieldValue(int row, const QVariant &value) {
...
void QmlPreferenceFormModel::save() {
  auto *manager = ServiceRegistry::instance()->rootItemManager();
