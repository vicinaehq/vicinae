...
QString QmlOAuthTokenStoreModel::displayTitle(const OAuth::TokenSet &set) const {
  return set.extensionId;
}

QString QmlOAuthTokenStoreModel::displaySubtitle(const OAuth::TokenSet &set) const {
  return set.providerId.value_or("");
}

QString QmlOAuthTokenStoreModel::displayIconSource(const OAuth::TokenSet &set) const {
  return imageSourceFor(ImageURL::builtin("key"));
}

QString QmlOAuthTokenStoreModel::displayAccessory(const OAuth::TokenSet &set) const {
  if (set.isExpired()) return QStringLiteral("Expired");
  return {};
}

std::unique_ptr<ActionPanelState>
QmlOAuthTokenStoreModel::buildActionPanel(const OAuth::TokenSet &set) const {
...
QString QmlOAuthTokenStoreModel::sectionLabel() const {
