...
QUrl VicinaeStoreDetailHost::qmlComponentUrl() const { return qml::componentUrl(u"StoreDetailView"); }
...
void VicinaeStoreDetailHost::createActions() {
  ...
  if (!m_isInstalled) {
    main->addAction(new InstallExtensionAction(QString("store.vicinae.%1").arg(m_ext.name),
                                               m_ext.themedIcon(),
                                               [url = m_ext.downloadUrl](const ApplicationContext *ctx) {
