...
class OpenSettingsPaneAction : public AbstractAction {
   public:
    ...
    void execute(ApplicationContext *ctx) override {
        ...
        if (!opened) {
            ctx->services->toastService()->failure(QCoreApplication::translate(
                "OpenSettingsPaneAction", "Failed to open System Settings"));
            return;
        }
...
std::unique_ptr<ActionPanelState> MacSettingsRootItem::newActionPanel(
    ApplicationContext *ctx, const RootItemMetadata &metadata) const {
    ...
    utils->addAction(
        new CopyToClipboardAction(Clipboard::Text(m_pane.bundleId), tr("Copy Bundle ID")));
...
RootProvider::Type MacSettingsRootProvider::type() const {
    return RootProvider::Type::GroupProvider;
}
