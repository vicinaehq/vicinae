...
class AIService {
	...
	dataReceived(
		handler: (completionId: string, data: string, done: boolean) => void,
	): EventSubscription {
		return this.transport.subscribe("AI/dataReceived", (msg) =>
			handler(msg.completionId, msg.data, msg.done),
		);
	}
}
