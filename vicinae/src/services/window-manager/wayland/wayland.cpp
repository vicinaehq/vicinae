...
static void foreign_toplevel_handle_output_enter(void *data,
    struct zwlr_foreign_toplevel_handle_v1 *handle, struct wl_output *output) {
  // ignore
}

static void foreign_toplevel_handle_output_leave(void *data,
    struct zwlr_foreign_toplevel_handle_v1 *handle, struct wl_output *output) {
  // ignore
}
...
static void foreign_toplevel_handle_parent(void *data,
    struct zwlr_foreign_toplevel_handle_v1 *handle,
...
WaylandWindow::WaylandWindow(WaylandWindowManager *wm,
    struct zwlr_foreign_toplevel_handle_v1 *handle) {
  m_manager = wm;
  m_handle = handle;
  m_pid = 0;
...
AbstractWindowManager::WindowList WaylandWindowManager::listWindowsSync() const {
    return m_toplevels;
}
...
bool WaylandWindowManager::sendShortcutSync(const AbstractWindow &window, const KeyboardShortcut &shortcut) {
  return false;
}
...
static void foreign_toplevel_manager_toplevel(void *data,
...
void WaylandWindowManager::start() {
  ...
  zwlr_foreign_toplevel_manager_v1_add_listener(
      m_manager, &foreign_toplevel_manager_listener, this);
