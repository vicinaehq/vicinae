...
std::optional<QDateTime> FileIndexerDatabase::getMaxLastModified(const std::filesystem::path &path) const {
  QSqlQuery query(m_db);
  query.prepare("SELECT MAX(last_modified_at) FROM indexed_file "
                "WHERE path LIKE :path_prefix ");
  query.bindValue(":path_prefix", qStringFromStdView(path.string()) + "%");

  if (!query.exec()) {
    qWarning() << "Failed to get max last_modified_at" << query.lastError();
    return std::nullopt;
  }
